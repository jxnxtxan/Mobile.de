...
(function () {
  ...
  function gmRegisterMenuCommand(caption, onClick) {
  ...
  function featureFlagsDefault() {
    const obj = {};
    FEATURE_FLAG_DEFINITIONS.forEach((d) => {
      obj[d.key] = !!d.default;
    });
  ...
  function addMissingDefaultEntries(userConfig, defaults) {
    ...
    console.info(
      `mobilede: ${missing.length} neue Default-Eintraege ergaenzt: ` + missing.map((m) => m.anzeige).join(", ")
    );
  ...
  function migrateAusstattungFavorit(userConfig) {
    ...
    let updated = false;
    const merged = userConfig.map((item) => {
      if (item.favorit !== void 0) return item;
      updated = true;
  ...
  function migrateAusstattungPreisGewicht(userConfig) {
    ...
    let updated = false;
    const merged = userConfig.map((item) => {
      if (item.preisGewicht !== void 0 && item.preisGewicht !== null) return item;
      const key = (item.anzeige || "").trim().toLowerCase();
  ...
  function applyPreisGewichtDefaults(userConfig, force) {
    ...
    return userConfig.map((item) => {
      ...
      const def = DEFAULT_PREIS_GEWICHT_BY_ANZEIGE[key];
      if (def == null) return item;
      if (typeof item.preisGewicht === "number" && item.preisGewicht > 0) return item;
      return { ...item, preisGewicht: def };
    });
  }
  ...
  function bootstrapConfig() {
    ...
    runtimeState.techDataKonfigurationen = ladeConfig(STORAGE_KEYS.techConfig) || techDataKonfigurationenDefault;
    runtimeState.mergeGruppenConfig = ladeConfig(STORAGE_KEYS.mergeGroups) || mergeGruppenConfigDefault;
  ...
  function isManualScope(scopeKey, flags) {
    const lo = getListOrder(runtimeState.featureFlags);
  ...
  function shouldApplyOrderToVehicleResults(flags) {
    const lo = getListOrder(runtimeState.featureFlags);
    return lo.mode === "manual" && !!lo.applyToVehicleResults && hasAnyManualListScope();
  }
  ...
  function sortEntriesByConfigOrder(entries, config, favoriteKeys, flags) {
    const lo = getListOrder(runtimeState.featureFlags);
    const useConfigOrder = lo.mode === "manual" && lo.applyToVehicleResults && (isManualScope("ausstattung") || isManualScope("ausstattungFavorites"));
    if (!useConfigOrder) {
      const out = [...entries].sort((a, b) => (a.anzeige || "").localeCompare(b.anzeige || "", "de"));
    ...
    const favOnly = isManualScope("ausstattungFavorites") && !isManualScope("ausstattung");
  ...
  function isForbiddenInWindow(tokens, window2, verboten) {
    if (!verboten || verboten.length === 0) return false;
    const slice = tokens.slice(window2.startIdx, window2.endIdx + 1).join(" ");
  ...
  function windowKey(srcId, window2) {
    return srcId + ":" + window2.startIdx + ":" + window2.endIdx;
  }
  ...
  function collectConfigMatches(sources, configs) {
    const candidates = [];
    const configList = isManualScope("ausstattung") || isManualScope("ausstattungFavorites") ? [...configs] : [...configs].sort((a, b) => (a.anzeige || "").localeCompare(b.anzeige || "", "de"));
    configList.forEach((cfg) => {
      ...
      for (const src of sources) {
        ...
        for (const begriff of cfg.begriffe) {
          ...
          const window2 = matchInTokens(src.tokens, parts, maxGap, compound);
          if (!window2) continue;
          if (cfg.verboten && cfg.verboten.length > 0) {
            const forbiddenParts = cfg.verboten.map((v) => cleanText(v)).filter(Boolean);
            if (isForbiddenInWindow(src.tokens, window2, forbiddenParts)) {
              continue;
            }
          }
          const snippetTokens = src.tokens.slice(
            Math.max(0, window2.startIdx - 2),
            Math.min(src.tokens.length, window2.endIdx + 3)
          );
          candidates.push({
            anzeige: cfg.anzeige,
  ...
  function enrichAussenMergeFromRaw(entries, rawItems) {
    const group = runtimeState.mergeGruppenConfig.find((g) => g && g.aktiv !== false && /aussenspiegel/.test(cleanText(g.basis || "")));
  ...
  function splitDescriptionIntoFeatures(rawText) {
    const normalized = rawText.replace(/\b(weitere ausstattung|sonderausstattung)\s*:/gi, ", ").replace(/\s{2,}/g, " ").trim();
  ...
  function consolidateAutoModeResults(entries, rawItems) {
    ...
    neutral.forEach((n) => {
      ...
      if (mergedHi.some((h) => rawCoveredByEntryLabel(label, h.anzeige))) {
        return;
      }
    ...
    if (mirrorNeutralLabels.length > 0) {
      ...
      const shortUniq = uniq.filter((l) => !mergedHi.some((h) => rawCoveredByEntryLabel(l, h.anzeige)));
  ...
  function dedupeConfigMatchesByAnzeige(matches) {
    ...
    for (const item of matches) {
      ...
      if (!existing) {
        byAnzeige.set(item.anzeige, item);
        continue;
      }
  ...
  function appendResultRow(columns, item, autoMode) {
    ...
    if (autoMode && !isHighlight) {
      ...
      const sourceLabel = isLow ? `Nur in Beschreibung gefunden (Quelle: ${item.source})` : `Quelle: ${item.source}`;
      ...
      const trigger2 = item.begriff ? `
Trigger: "${item.begriff}"` : "";
      const snippet = item.snippet ? `
Kontext: …${item.snippet}…` : "";
      span.title = sourceLabel + inactiveNote + trigger2 + snippet;
      if (isLow) {
        span.style.fontStyle = "italic";
  ...
  function ergebnisHinzufuegen(force) {
    ...
    if (gefundeneTexte.length > 0) {
      ...
      const favCount = gefundeneTexte.filter((i) => favKeys.has((i.anzeige || "").trim().toLowerCase())).length;
      ...
      keine.textContent = autoMode ? "Keine Ausstattungseinträge auf der Seite gefunden." : "Keine der gesuchten Begriffe gefunden.";
  ...
  function extractIdsFromAd(ad) {
    ...
    const makeId = resolveNumericId(
      ad.makeId ?? ((_a = ad.make) == null ? void 0 : _a.id) ?? ((_b = ad.make) == null ? void 0 : _b.key) ?? ((_c = ad.vehicle) == null ? void 0 : _c.makeId)
    );
    const modelId = resolveNumericId(
      ad.modelId ?? ((_d = ad.model) == null ? void 0 : _d.id) ?? ((_e = ad.model) == null ? void 0 : _e.key) ?? ((_f = ad.vehicle) == null ? void 0 : _f.modelId)
    );
    const modelGroupId = resolveNumericId(
      ad.modelGroupId ?? ((_g = ad.modelGroup) == null ? void 0 : _g.id) ?? ((_h = ad.vehicle) == null ? void 0 : _h.modelGroupId)
    );
  ...
  function buildVehicleProfileFromAd(ad, adId) {
    ...
    return {
      ...
      priceGross: ((_a = ad.price) == null ? void 0 : _a.grossAmount) ?? parseEuroAmount((_b = ad.price) == null ? void 0 : _b.gross),
  ...
  function buildCohortSearchUrl(profile, prCfg) {
    ...
    const ms = useModelRange ? profile.searchMs || formatMsParam(profile.makeId, profile.modelId, modelGroupId) : formatMsParam(profile.makeId, profile.modelId, null);
    if (ms) {
    ...
    const powerKw = profile.powerKw != null ? profile.powerKw : profile.powerPs != null ? Math.round(profile.powerPs * PS_TO_KW) : null;
    if (powerKw != null) {
  ...
  function prunePriceDataStore(store, nowTs) {
    ...
    const cohorts = Object.entries(store.cohortsByKey || {}).filter(([, c]) => c && typeof c.ts === "number" && now - c.ts <= maxAge && Array.isArray(c.adIds) && c.adIds.length > 0).sort((a, b) => (b[1].ts || 0) - (a[1].ts || 0));
    const keptCohorts = cohorts.slice(0, PRICE_DATA_STORE_MAX_COHORTS);
    store.cohortsByKey = Object.fromEntries(keptCohorts);
    const anchors = Object.entries(store.anchorsByAdId || {}).filter(([, a]) => a && typeof a.ts === "number" && now - a.ts <= maxAge).sort((a, b) => (b[1].ts || 0) - (a[1].ts || 0)).slice(0, 300);
    ...
    const ads = Object.entries(store.adsById || {}).filter(([id, ad]) => keepAdIds.has(String(id)) && ad && typeof ad === "object").sort((a, b) => (b[1].ts || 0) - (a[1].ts || 0)).slice(0, PRICE_DATA_STORE_MAX_ADS);
  ...
  function profileFromSearchPageUrl(href) {
    try {
      ...
      if (msValues.length) {
        ...
        const parsedList = msValues.map((v) => parseMsParam(v)).filter(Boolean);
  ...
  function syncCohortCacheFromSearchPage() {
    ...
    notifyCohortCacheUpdated();
    const top = [...byKey.entries()].sort((a, b) => b[1].length - a[1].length).slice(0, 6).map(([key, list]) => ({ cacheKey: key, count: list.length }));
  ...
  function findSrpListingsInState(state) {
    ...
    const tryList = (v) => {
      ...
      if (typeof v === "object") {
        ...
        if (vals.length && vals.every((x) => x && (x.id || x.ad || x.data))) {
          return vals.map((x) => {
            var _a2;
            return x.ad || ((_a2 = x.data) == null ? void 0 : _a2.ad) || x;
          });
        }
      }
      return [];
    ...
    const explicitPaths = [
      (_b = (_a = state == null ? void 0 : state.search) == null ? void 0 : _a.srp) == null ? void 0 : _b.ads,
      (_d = (_c = state == null ? void 0 : state.search) == null ? void 0 : _c.srp) == null ? void 0 : _d.items,
      (_f = (_e = state == null ? void 0 : state.search) == null ? void 0 : _e.srp) == null ? void 0 : _f.searchResults,
      (_h = (_g = state == null ? void 0 : state.search) == null ? void 0 : _g.srp) == null ? void 0 : _h.results,
      (_j = (_i = state == null ? void 0 : state.search) == null ? void 0 : _i.srp) == null ? void 0 : _j.resultList,
      (_l = (_k = state == null ? void 0 : state.search) == null ? void 0 : _k.srp) == null ? void 0 : _l.listings,
      (_m = state == null ? void 0 : state.search) == null ? void 0 : _m.results,
      (_n = state == null ? void 0 : state.search) == null ? void 0 : _n.resultList,
      (_o = state == null ? void 0 : state.search) == null ? void 0 : _o.listings
    ];
    ...
    const isAdLike = (entry) => {
      ...
      const ad = (entry == null ? void 0 : entry.ad) || ((_a2 = entry == null ? void 0 : entry.data) == null ? void 0 : _a2.ad) || entry;
      if (!ad || typeof ad !== "object") return false;
      return !!(ad.id || ad.adId || ad.mobileAdId) && !!(ad.price || ad.prices || ad.make || ad.model || ad.title || ad.vehicleDescription);
    };
    let best = [];
    const queue = [(_p = state == null ? void 0 : state.search) == null ? void 0 : _p.srp, state == null ? void 0 : state.search, state].filter(Boolean);
    const seen = typeof WeakSet !== "undefined" ? new WeakSet() : null;
    let visited = 0;
    const MAX_OBJECTS = 4e3;
  ...
  function computePriceRating(profile, comparables, options) {
    ...
    if (prices.length >= prCfg.minComparables) {
      ...
      priceRatingDebugLog("Baseline aus mobile-Fallback", {
        cohortCount,
        mobileLabel: ((_a = profile.priceRating) == null ? void 0 : _a.ratingLabel) || null,
    ...
    const topBreakdown = [...ownEquip.breakdown].sort((a, b) => b.weight - a.weight).slice(0, 5);
    return {
      ok: true,
      label,
      level,
  ...
  function formatCohortCountText(rating, opts) {
    ...
    let txt = forModal ? rating.cohortCount + " Vergleichsfahrzeuge" : "Vergleich: " + rating.cohortCount + " Fahrzeuge";
...
`;
  ...
  function openPriceRatingModal(rating, profile) {
    ...
    if (rating.needsCohortSearch) {
      ...
      pBlock.textContent = "Für den Median-Basispreis die Vergleichssuche öffnen und auf der Suchergebnisseite bleiben (Liste muss laden). Geöffnete Inserate helfen zusätzlich für präzisere Ausstattungs-Scores (inkl. Beschreibungstext), erhöhen aber nicht die Kohortenanzahl. Die Trefferliste wird lokal zwischengespeichert (auch im neuen Tab) — keine Hintergrund-Anfragen.";
  ...
  function renderVipPriceRatingWidget(rating, loading) {
    ...
    const inSidebar = anchor.classList.contains("wNWsk") || !!anchor.closest("aside.iKWwq");
    ...
    if (!rating || !rating.ok) {
      ...
      if (rating && rating.needsCohortSearch) {
        ...
        lab.textContent = rating && rating.reason === "no_baseline" ? "Preisbewertung: zu wenig Vergleichsdaten" : "Preisbewertung nicht verfügbar";
      }
      row.appendChild(lab);
      if (rating && rating.needsCohortSearch) {
      ...
      if (cached && cached.ok && !cached.needsCohortSearch && ratingCohortMetaMatches(cached, cohortRes)) {
        if (cohortRes.items.length || !cached.usedMobileFallback) {
          ...
          priceRatingDebugLog("Preisbewertung aus Session-Cache gerendert", {
            adId: profile.id,
            cohortCount: rating2.cohortCount,
            usedMobileFallback: rating2.usedMobileFallback
          });
  ...
  function loadSrpCardRating(card) {
    ...
    computeAndCacheRatingForProfileAsync(profile).then((rating) => {
      ...
      card.dataset.mobiledePriceRated = "1";
      renderSrpPriceBadge(card, rating);
    }).catch(() => {
      delete card.dataset.mobiledePriceRated;
    });
  }
  ...
  function runScheduledTasks() {
    ...
    const items = [...scheduledJobs.entries()].sort((a, b) => (taskPriority[a[1].type] ?? 99) - (taskPriority[b[1].type] ?? 99));
    scheduledJobs.clear();
    let deferredCount = 0;
  ...
  function isIgnoredNode(node) {
    ...
    if ((_a = node.matches) == null ? void 0 : _a.call(node, IGNORE_SELECTOR)) return true;
    if ((_b = node.closest) == null ? void 0 : _b.call(node, IGNORE_SELECTOR)) return true;
    const id = node.id || "";
  ...
  function isRelevantContentElement(el) {
    ...
    if (isSearchResultsPage()) return !!((_a = el.closest) == null ? void 0 : _a.call(el, SRP_CONTENT_SELECTOR));
    if (isVehicleDetailPage()) return !!((_b = el.closest) == null ? void 0 : _b.call(el, VIP_CONTENT_SELECTOR));
    return !!((_c = el.closest) == null ? void 0 : _c.call(el, "main, article"));
  }
  function collectRelevantAddedRoots(mutations) {
    const roots = new Set();
  ...
  function mutationAffectsPageContent(mutation) {
    if (mutation.type === "characterData") {
      const el2 = mutation.target.parentElement;
      return isRelevantContentElement(el2);
    }
  ...
  function computeCohortSyncSignature() {
    if (!isSearchResultsPage()) return "";
    const state = getPageInitialState();
...
<li><strong>Preisbewertung:</strong> Vollständig im Tab <strong>Config</strong> — Schwellen, €/Punkt, Vergleichskohorte, Ausstattungs-Gewichte. Änderungen mit starker Auswirkung fragen per Warnung nach.</li>
  ...
  function oeffneKonfigPopup() {
    ...
    let aktuelleAusstattungsKonfig = JSON.parse(JSON.stringify(runtimeState.suchKonfigurationen));
    let aktuelleTechKonfigurationen = JSON.parse(JSON.stringify(runtimeState.techDataKonfigurationen));
    let aktuelleMergeGruppen = JSON.parse(JSON.stringify(runtimeState.mergeGruppenConfig));
    let aktuelleFeatureFlags = mergeConfigListUi(
      runtimeState.featureFlags || {},
      { ...featureFlagsDefault(), ...runtimeState.featureFlags || {} }
    );
    ...
    function diffListOrder(baseline, current) {
      ...
      if (b.mode !== c.mode) {
        lines.push("Listen-Reihenfolge: Modus " + (b.mode === "manual" ? "manuell" : "alphabetisch") + " → " + (c.mode === "manual" ? "manuell" : "alphabetisch"));
      }
      const scopeLabels = {
      ...
      Object.keys(scopeLabels).forEach((k) => {
        if (!!b.scopes[k] !== !!c.scopes[k]) {
          lines.push("Listen-Reihenfolge: " + scopeLabels[k] + " manuell " + (c.scopes[k] ? "an" : "aus"));
        }
      });
      if (!!b.applyToVehicleResults !== !!c.applyToVehicleResults) {
        lines.push("Listen-Reihenfolge: Fahrzeugseite " + (c.applyToVehicleResults ? "übernimmt Reihenfolge" : "alphabetisch/Favoriten"));
      }
      return lines;
    }
    ...
    function diffPriceRating(baseline, current) {
      ...
      if (b.maxAdjustPct !== c.maxAdjustPct) {
        lines.push("Preisbewertung max. Korrektur: " + Math.round(b.maxAdjustPct * 100) + "% → " + Math.round(c.maxAdjustPct * 100) + "%");
      }
      ...
      b.thresholds.forEach((bt, i) => {
        ...
        lines.push("Preisbewertung Schwelle „" + PRICE_RATING_LEVELS[i].label + "“: " + fmt(bt.maxPct) + " → " + fmt(ct.maxPct));
      });
      return lines;
    }
...
`;
    ...
    function mkToolbarZone(label, stack) {
      ...
      zone.className = "mc-toolbar-zone";
    ...
    function buildListToolbar(opts) {
      ...
      const searchZone = mkToolbarZone("Suche");
      ...
      if (filters.length || bulk) {
        ...
        if (filters.length) {
          const displayZone = mkToolbarZone("Anzeige");
        ...
        if (bulk) {
          const bulkZone = mkToolbarZone("Alle Einträge");
    ...
    function mkHelpPanel(htmlContent) {
      ...
      head2.className = "mc-help-panel__head";
      const closeBtn2 = document.createElement("button");
      closeBtn2.type = "button";
      closeBtn2.className = "mc-icon-btn mc-help-panel__close";
      closeBtn2.setAttribute("aria-label", "Hilfe schließen");
      closeBtn2.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.3 5.71a1 1 0 00-1.41 0L12 10.59 7.11 5.7A1 1 0 105.7 7.11L10.59 12 5.7 16.89a1 1 0 101.41 1.41L12 13.41l4.89 4.89a1 1 0 001.41-1.41L13.41 12l4.89-4.89a1 1 0 000-1.4z"/></svg>';
      const body = document.createElement("div");
      body.className = "mc-help-panel__body";
      body.innerHTML = htmlContent;
      head2.appendChild(closeBtn2);
      wrap.appendChild(head2);
      wrap.appendChild(body);
      return { wrap, closeBtn: closeBtn2, body };
    }
    ...
    function installKonfigTabHelp(baseKey, panelId, regionAriaLabel, btnLabel, toolbarEl, metaEl, panelColumn, beforeNode) {
      const html = KONFIG_TAB_HELP_HTML.get(konfigHelpKey(baseKey));
      if (!html) return;
      const { wrap, closeBtn: closeBtn2, body } = mkHelpPanel(html);
      wrap.id = panelId;
    ...
    function mkConfigSplitShell(tabId) {
      ...
      const sheetDone = mkBtn("ghost", "Fertig", () => closeEditorSheet());
    ...
    function mkDragHandle(enabled) {
      ...
      h.textContent = "⋮⋮";
      h.title = enabled === false ? "Manuelle Reihenfolge im Config-Tab aktivieren (Bereich + Modus „Manuell“)" : "Ziehen zum Sortieren (nur vertikal)";
      h.draggable = false;
      return h;
    }
    function setupListDragReorder(opts) {
      ...
      function listCards() {
        return [...container.querySelectorAll(".mc-card[" + indexAttr + "]")].filter((c) => !c.classList.contains("mc-card--drag-source-hidden"));
      }
      ...
      function getScrollParent() {
        ...
        while (p) {
          ...
          if (overflow > bestOverflow) {
            ...
            if (oy === "auto" || oy === "scroll" || oy === "overlay" || p.classList.contains("mc-popup__scroll")) {
              best = p;
      ...
      function getPopupClipRect() {
        const popup2 = container.closest(".mc-popup");
        return popup2 ? popup2.getBoundingClientRect() : null;
      }
      ...
      function createFloatPreview(pointerY) {
        ...
        const popup2 = container.closest(".mc-popup");
        (popup2 || document.body).appendChild(floatPreview);
      ...
      function computeToIndex() {
        ...
        while (next) {
          if (next.matches && next.matches(".mc-card[" + indexAttr + "]") && !next.classList.contains("mc-card--drag-source-hidden")) {
    ...
    function sortIndices(indices, items, key, dir, valueFn) {
      ...
      return [...indices].sort((ia, ib) => {
        ...
        if (typeof va === "boolean" || typeof vb === "boolean") {
          ...
          cmp = String(va ?? "").localeCompare(String(vb ?? ""), void 0, { sensitivity: "base" });
        }
    ...
    function mkColSortBtn(label, sortKey, title2, getState, setState, onChange, opts) {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "mc-col-sort-btn" + (opts && opts.left ? " mc-col-sort-btn--left" : "");
      btn.title = title2 || "Nach „" + label + "“ sortieren (Klick wechselt ↑/↓)";
      function syncBtn() {
        const s = getState();
    ...
    function confirmAsync(msg) {
      return new Promise((resolve) => {
        ...
        const no = mkBtn("ghost", "Abbrechen", () => {
          back.remove();
          resolve(false);
        });
    ...
    function confirmSaveWithChangelog(changes) {
      return new Promise((resolve) => {
        ...
        const yes = mkBtn("primary", "Speichern", () => {
          back.remove();
          resolve(true);
        });
        row.appendChild(no);
    ...
    function removeOverlay() {
      ...
      try {
        delete pageWindow.__mobiledeDragDiag;
      } catch (_e) {
    ...
    function snapshotAus() {
      return JSON.parse(JSON.stringify(aktuelleAusstattungsKonfig));
    }
    function snapshotTech() {
      return JSON.parse(JSON.stringify(aktuelleTechKonfigurationen));
    }
    ...
    function ausCompareValue(item, idx, key) {
      switch (key) {
        case "anzeige":
          return (item.anzeige || "").trim();
        case "aktiv":
          return item.aktiv === true;
        case "favorit":
          return item.favorit === true;
        case "farbe":
          return normalizeHexColor(item.farbe || "");
        case "nurInFeatures":
          return item.nurInFeatures === true;
        case "compound":
          return item.compound === true;
        case "begriffeCount":
          return Array.isArray(item.begriffe) ? item.begriffe.length : 0;
        case "config":
        default:
          return idx;
      }
    ...
    function fillAusEditor(idx) {
      ...
      title2.className = "mc-config-split__editor-title";
      title2.textContent = (item.anzeige || "").trim() || "Neuer Eintrag";
      editor.appendChild(title2);
    ...
    function cardIssuesAus(idx, item) {
      ...
      const key = (item.anzeige || "").trim().toLowerCase();
      if (key) {
        const dup = aktuelleAusstattungsKonfig.findIndex((other, j) => j !== idx && (other.anzeige || "").trim().toLowerCase() === key);
    ...
    function sanitizeExpandedAusstattungIndex() {
      ...
      const n = aktuelleAusstattungsKonfig.length;
      if (!Number.isInteger(expandedAusstattungIndex) || expandedAusstattungIndex < 0 || expandedAusstattungIndex >= n) {
        expandedAusstattungIndex = null;
      }
    }
    function applyAusAccordionStateToAusCards() {
      ...
      ausstattungContainer.querySelectorAll(".mc-card").forEach((card) => {
        ...
        if (eb) {
          ...
          eb.setAttribute(
            "aria-label",
            (isOpen ? "Details-Bereich verbergen. Blendet die Felder für Suchbegriffe und verbotene Wörter aus." : "Details-Bereich anzeigen. Öffnet die Felder zum Bearbeiten von Suchbegriffen und verbotenen Wörtern.") + " Aktuell " + nPart + " " + begriffeW + " und " + vPart + " " + verboteW + "."
          );
        }
    ...
    function appendAusstattungCard(index, dragSection) {
      ...
      function updateExpandButton() {
        ...
        expandBtn.setAttribute(
          "aria-label",
          (isOpen ? "Details-Bereich verbergen. Blendet die Felder für Suchbegriffe und verbotene Wörter aus." : "Details-Bereich anzeigen. Öffnet die Felder zum Bearbeiten von Suchbegriffen und verbotenen Wörtern.") + " Aktuell " + n + " " + begriffeW + " und " + v + " " + verboteW + "."
        );
      }
    ...
    function fillTechEditor(idx) {
      ...
      const title2 = document.createElement("div");
      title2.className = "mc-config-split__editor-title";
      title2.textContent = (item.begriff || "").trim() || "Neuer Tech-Eintrag";
      editor.appendChild(title2);
      const fields = document.createElement("div");
      fields.className = "mc-config-split__editor-fields";
      const inp = document.createElement("input");
    ...
    function getSortedTechVisibleIndices() {
      ...
      return isPopupManualScope("tech") ? orderIndicesByArrayPosition(vis) : sortIndices(vis, aktuelleTechKonfigurationen, techSortKey, techSort.dir, techCompareValue);
    }
    ...
    function mergeCompareValue(group, idx, key) {
      switch (key) {
        case "basis":
          return (group.basis || "").trim();
        case "aktiv":
          return group.aktiv !== false;
        case "orderCount":
          return Array.isArray(group.order) ? group.order.length : 0;
        case "config":
        default:
          return idx;
      }
    ...
    function fillMergeEditor(idx) {
      ...
      const title2 = document.createElement("div");
      title2.className = "mc-config-split__editor-title";
      title2.textContent = (group.basis || "").trim() || "Neue Merge-Gruppe";
      editor.appendChild(title2);
      const fields = document.createElement("div");
      fields.className = "mc-config-split__editor-fields";
    ...
    ieIntro.innerHTML = "<strong>Backup &amp; Teilen:</strong> Konfiguration als JSON exportieren oder einspielen. Vor dem Import wird automatisch ein Backup angelegt — per <strong>Rückgängig</strong> im Footer wiederherstellbar. Schema <strong>v" + SCHEMA_VERSION + "</strong>.";
    ...
    drop.addEventListener("dragover", (e) => {
      e.preventDefault();
      drop.classList.add("mc-dropzone--hover");
    });
    ...
    storeDrop.addEventListener("dragover", (e) => {
      e.preventDefault();
      storeDrop.classList.add("mc-dropzone--hover");
    });
    ...
    configIntro.addEventListener("click", () => {
      ...
      configDebugUnlockTimer = setTimeout(() => {
    ...
    function renderConfig() {
      ...
      function appendFeaturesSection() {
        ...
        FEATURE_FLAG_DEFINITIONS.forEach((def) => {
          ...
          const title2 = document.createElement("div");
          title2.className = "mc-feature-title";
          title2.textContent = def.title;
          const desc = document.createElement("div");
          desc.className = "mc-feature-desc";
          desc.textContent = def.description || "";
          txtCol.appendChild(title2);
      ...
      function syncListOrderUi() {
        ...
        const hasScope = manual && (lo.scopes.ausstattung || lo.scopes.ausstattungFavorites || lo.scopes.tech);
      ...
      async function confirmMasterPrToggle(enabling) {
        if (enabling) {
          return confirmAsync(
            "Preisbewertung aktivieren (Beta)\n\nDie ausstattungsbereinigte Preisbewertung ist experimentell. Bewertungen und Schwellen können sich mit Skript-Updates ändern. Geplant ist, die Funktion später dauerhaft ohne Beta-Kennzeichnung anzubieten.\n\nJetzt aktivieren?"
          );
        }
        return confirmAsync(
          "Preisbewertung deaktivieren\n\nEs werden keine Preis-Badges mehr auf Fahrzeugdetail- und Suchergebnisseiten angezeigt. Deine Einstellungen bleiben gespeichert.\n\nJetzt deaktivieren?"
        );
      }
      ...
      pr.thresholds.slice(0, 4).forEach((th, i) => {
        ...
        inp.addEventListener("change", async () => {
          ...
          if (next === prev) return;
          const ok = await confirmPrImpact(
            "Schwelle „" + PRICE_RATING_LEVELS[i].label + "“: " + Math.round(prev * 100) + "% → " + Math.round(next * 100) + "%. Verschiebt die gesamte 5-Stufen-Einteilung."
          );
          if (!ok) {
            inp.value = String(Math.round(prev * 100));
            return;
    ...
    statusBtn.addEventListener("click", () => {
      ...
      issuePop.innerHTML = "<strong>Validierung</strong><ul>" + allIssues.slice(0, 40).map((t) => "<li>" + t.replace(/</g, "&lt;") + "</li>").join("") + (allIssues.length > 40 ? "<li>…</li>" : "") + "</ul>";
  ...
  bootstrapConfig();
  initApp();
  setTimeout(erstelleKonfigButton, 3e3);
